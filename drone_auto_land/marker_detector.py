...
class MarkerDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        if len(corners) > 0:
            ...
            match len(ids):
                case 1:
                    rvec, tvec = self.estimate_pose(corners, (self.marker_size if ids[0][0] == self.marker_id else self.embedded_marker_size))
                case 2:
                    # Check which of the corners is the big marker and estimate its pose
                    if ids[0][0] == self.marker_id:
                        rvec, tvec = self.estimate_pose([corners[0]], self.marker_size)
                    else:
                        rvec, tvec = self.estimate_pose([corners[1]], self.marker_size)
                case _:
